...
### 新增
- **成本预测**：7 天线性回归 → 预测下月成本（服务详情页）
- **异常检测**：均值 ± 2σ 检测 CPU 突增突降（服务详情页）
- **HPA 建议**：基于 P10/P50/P95 流量分析推荐 min/max replicas（服务详情页）
- **Pod 稳定性报告**：重启次数 / OOMKill / CrashLoopBackOff（健康状态页）
- **入口流量排行**：Top 入口 + Top 服务 24h RPS（集群分析页）
- **慢请求分析**：P99 > 100ms 服务列表 + Jaeger Trace 链接（健康状态页）
- **依赖风险检测**：拓扑图标记单点服务 ⚠️
- **暗色模式**：全页面适配 `[data-theme="dark"]`

### 变更
- 总览页图表重构：堆积条形图、指标卡片、蝴蝶图、环形图中心数字
- 资源推荐页大改：节省优先卡片，默认按节省排序
- 流量效率页大改：浪费优先卡片 + 黑洞高亮
- 健康状态页大改：严重度优先卡片 + 评分进度条
- 集群分析页大改：伸缩横幅 + 利用率色条
- 导航栏 CNDP 风格：白底毛玻璃 + 轻量链接
- 服务详情页新增成本趋势图 + 预测/异常/HPA 卡片
